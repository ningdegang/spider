...
def main():
    session = requests.get(url=url, proxies=proxy)
    html = session.content.decode('utf-8', 'ignore')
    #print html
    doc = PyQuery(html, parser='html')
    playing = doc("div.span9 div.movie-index-poster a[target='_blank']")
    play = list()
    for a in playing.items(): 
        play.append(a.attr["href"])
        print a.attr["href"]
    play = list(set(play))
    print("total film: ",  len(play))
